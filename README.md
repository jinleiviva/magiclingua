...
装好扩展、本地服务在跑即可。点插件图标进入**主视图**：顶部是**启用翻译总开关**与**本地模型状态（启动/停止）**，往下是站点能力开关、整页翻译、字幕导出（视频页自动出现）、目标语言与显示模式；点右上角 **⚙️** 进入**设置视图**（翻译行为 / 译文样式 / 术语表 / 服务地址），底部入口直达文档翻译助手。

开启「启用翻译」或点「启动」拉起模型后，**当前页面会自动开始翻译**——每个译文将出现的位置先显示一个转圈提示，译文生成后原位替换；关闭开关则移除全部译文、恢复原文，全程无需手动刷新页面。
